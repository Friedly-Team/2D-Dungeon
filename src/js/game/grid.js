...
class Grid {
    ...
    for (let y = 0; y < this.yTotal; y++) {
      ...
      for (let x = 0; x < this.xTotal; x++) {
        if ((x === 0 && y < this.yTotal) || (y === 0 && x < this.xTotal)) {
          ...
          (x === this.xTotal - 1 && y < this.yTotal) ||
          (y === this.yTotal - 1 && x < this.xTotal)
        ) {
          this.data[y].push({ x, y, fill: 55, type: "wall" });
        } else {
  ...
  show() {
    this.data.forEach((row) =>
      row.forEach((b) => this.#drawTile(b, this.g.metaTiles[b.type]))
    );
  }
  ...
  #drawTile(block, sprite) {
    ...
    this.g.image(
      this.g.tileset,
      block.x * scale,
      block.y * scale, // pos on grid in px
      scale,
      scale, // size in px
      sprite.x,
      sprite.y,
      31,
      31 // pos & size in tileset
    );
  }
